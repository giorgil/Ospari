...
<div class="row draft-item" id="row-<?php echo $draft->id ?>"  draft-id="<?php echo $draft->id ?>">
    ...
    <div class="col-lg-2 edit-date">
        <?php echo $draft->edited_at ?>
    </div>
    <div class="col-lg-2 blog-status">
        <?php if( $draft->isPublished() ) {echo 'published'; }else{ echo 'draft'; }  ?>
    </div>
    <div class="col-lg-1 blog-action">
       <?php if($draft->isPublished()):?>
            <a href="#" class="blog-tooltip" data-toggle="tooltip" data-placement="top" title data-original-title="Click to unpublish this post" onclick=" return try_unpublish(<?php echo $draft->id?>,'/<?php echo OSPARI_ADMIN_PATH ?>');"><i class="fa fa-eye-slash"></i></a>
       <?php else:?>
